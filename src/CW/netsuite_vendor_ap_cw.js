...
async function putVendor(connections, vendorData, vendor_id) {
  try {
    ...
    const formatData = {
      ...
      companyName:
        vendorData?.companyname.length > 0
          ? vendorData?.companyname.replace(/'/g, "`")
          : "",
      ...
      legalName: vendorData?.legalname?.replace(/'/g, "`"),
